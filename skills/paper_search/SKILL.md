...
`~/.workbuddy/skills/paper_search/scripts/search_papers.py` �?invoke
...
```bash
SEARCH="~/.workbuddy/skills/paper_search/scripts/search_papers.py"
```
...
3. **Key themes**: 3�? main research themes / clusters across all results,
   each with a one-line description and 2�? representative paper numbers.
...
7. **Recommendations for reading**: 3�? papers most relevant and impactful to the user's
...
no script call. Instead, after the CLI search returns, recall 5�?0 additional
...
recall �? papers with reasonable confidence, return fewer; an empty
