...
Why captions matter: a good article lives or dies on picking the right figure (the
overview diagram, the headline results plot) and skipping decorative or
equation-as-image junk. Captions are the cheapest, most reliable signal of what a
figure actually shows, so we extract them alongside the images and let the model
match figures to captions when planning figure placement in the article.
"""
...
def extract_figures_pymupdf(pdf: Path, figdir: Path, text: str) -> list[dict]:
    ...
    Strategy — caption-anchored, cluster-based region detection:
      ...
      4. Crop just the figure region at 2× zoom (~144 dpi); caption text is
...
def parse_metadata(text: str) -> dict:
    ...
    if a field can't be extracted, leave it empty. The article should simply
    omit that element when a field is missing — never fabricate or placeholder it.
    """
    pages = text.split("\f")
...
def main():
    ...
    figures_dir = outdir / "assets" / "figures"
    meta_dir.mkdir(parents=True, exist_ok=True)
    figures_dir.mkdir(parents=True, exist_ok=True)

    print(f"Extracting text from {pdf.name}...")
    text_path = meta_dir / "text.txt"
    text = extract_text(pdf, text_path)
    nchars = text_path.stat().st_size
    print(f"  assets/meta/text.txt: {nchars} chars")

    print("Parsing captions...")
    captions = parse_captions(text)
    (meta_dir / "captions.json").write_text(json.dumps(captions, indent=2))
    print(f"  {len(captions)} captions parsed -> assets/meta/captions.json")
    ...
    print(f"  assets/meta/metadata.json populated fields: {populated or '(none)'}")
    ...
    figs = extract_figures_pymupdf(pdf, figures_dir, text)
    (meta_dir / "figures.json").write_text(json.dumps(figs, indent=2))
    print(f"  {len(figs)} figures saved to {figures_dir}")
    manifest = {
        "schema_version": "paper2blog_extract.v1",
        "layout": "v2-assets",
        "created_at": utc_now(),
        "files": {
    ...
    print("  manifest.json written with layout=v2-assets")
    if figs:
        for f in figs[:8]:
