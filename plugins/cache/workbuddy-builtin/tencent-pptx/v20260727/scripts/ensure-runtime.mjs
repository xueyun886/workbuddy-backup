 ...
 *      Unix:    ~/.workbuddy/binaries/node/versions/<v>/bin/<bin>
 *               ~/.workbuddy/binaries/node/versions/<v>/lib/node_modules/<pkg>/
 *      Windows: ~/.workbuddy/binaries/node/versions/<v>/<bin>.cmd
 *               ~/.workbuddy/binaries/node/versions/<v>/node_modules/<pkg>/
 *    用 `npm install -g --prefix=<nodeRoot>` 锁定，避免被 ~/.npmrc 劫持到 nvm/系统全局。
 *  - slidep 以 vendor tar.gz 随插件分发，首次使用时本地 npm install，无运行时网络拉取。
 */
...
const SLIDEP_VENDOR_TARBALL = join(PLUGIN_ROOT, 'vendor', 'tencent-slidep-5.4.4.tar.gz');
...
const NPM_EXE = IS_WIN ? 'npm.cmd' : 'npm';
...
function resolveNodeBin() {
  ...
  if (fromEnv && existsSync(join(fromEnv, NODE_EXE))) {
    // Windows: <root>/node.exe → <root>；Unix: <root>/bin/node → <root>/..
    const nodeRoot = IS_WIN ? fromEnv : (dirname(fromEnv) === '/' ? fromEnv : dirname(fromEnv));
    return {
      nodeBin: join(fromEnv, NODE_EXE),
      npmBin: join(fromEnv, NPM_EXE),
      nodeRoot,
      source: 'NODE_BIN_DIR',
    };
  }
  ...
  if (existsSync(wbVersionsDir)) {
    try {
      ...
      for (const v of dirs) {
        ...
        // Windows: node.exe 直接在 root 下；Unix: 在 root/bin/ 下
        const binDir = IS_WIN ? root : join(root, 'bin');
        if (existsSync(join(binDir, NODE_EXE))) {
          return {
            nodeBin: join(binDir, NODE_EXE),
            npmBin: join(binDir, NPM_EXE),
            nodeRoot: root,
            source: `workbuddy@${v}`,
          };
        }
  ...
  return { nodeBin: NODE_EXE, npmBin: NPM_EXE, nodeRoot: null, source: 'PATH' };
}
...
function pkgPathsFor(nodeRoot, pkg) {
  ...
  // Windows: npm -g --prefix 安装到 <prefix>/node_modules/<pkg>/
  // Unix:    npm -g --prefix 安装到 <prefix>/lib/node_modules/<pkg>/
  const modulesBase = IS_WIN
    ? join(nodeRoot, 'node_modules', pkg.dirName)
    : join(nodeRoot, 'lib', 'node_modules', pkg.dirName);
  return {
    bin: IS_WIN
      ? join(nodeRoot, pkg.binName + '.cmd')
      : join(nodeRoot, 'bin', pkg.binName),
    pkgJson: join(modulesBase, 'package.json'),
    home: nodeRoot,
  };
}
...
function installPackage(npmBin, nodeRoot, pkg) {
  ...
  // Windows 上 npm.cmd 需要通过 shell 执行；Git Bash 环境下虽非必须但无害。
